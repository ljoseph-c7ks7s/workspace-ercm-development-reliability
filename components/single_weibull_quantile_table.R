fn <- function(conn_rmysql, load_path, params_path, component_path, output_file) {
  ...
  df <- dbGetQuery(conn_rmysql, paste0("SELECT interval_value, causal FROM `interval` WHERE interval_value >= ", min_interval_time))
  
  df_for_fits <- Surv(df$interval_value, df$causal)
  
  single_fit <- flexsurvreg(df_for_fits ~ 1, dist="weibull")
  ...
  to_return <- bind_rows(df_ci_tow, df_ci_qntl)
