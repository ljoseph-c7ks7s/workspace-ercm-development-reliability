fn <- function(conn_rmysql, load_path, params_path, component_path, output_file) {
  ...
  df_cumulate$Land_Date <-  df_cumulate$Land_Date %>% substr(1,10) %>% as.POSIXct()
  df_cumulate$Land_Time <- substr(df_cumulate$Land_Time %>% as.POSIXct(format = "%H:%M:%OS"),12,16)
  df_cumulate$Land_Date <- as.POSIXct(paste(df_cumulate$Land_Date, df_cumulate$Land_Time), format = "%Y-%m-%d %H:%M")
  df_cumulate$Land_Date <- df_cumulate$Land_Date %>% as.character()
