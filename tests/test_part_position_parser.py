...
def test_part_position_parser():
    ...
    df_qpa = pd.read_csv('../data/wuc_list_qpa_translated.csv',dtype={'Names':'str'})
    ...
    df_output.loc[:,'Component_Position_Number'] = df_output.loc[:,'Component_Position_Number'].map(lambda x: '0' if x=='nan' else x)
