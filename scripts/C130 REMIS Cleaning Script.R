...
con2 <- dbConnect(drv = MySQL(),
                 dbname="ercm",
                 username="root",
                 password="",
                 host="localhost",
                 port=3306)
df_full <- dbGetQuery(con2, 'SELECT * FROM ercm.sortie_merged')
dbDisconnect(con2)
...
data_edit$Transaction_Date <- lubridate::ymd(data_edit$Transaction_Date) ##From REMIS

date_check <- lubridate::ymd_hms(df_cumulate$Depart_Date) %>% as.data.frame() %>% unique() ##From Sortie
